  ...
  useEffect(() => {
    const fetchPreviews = async () => {
      ...
      try {
        ...
        const previewUrls = await spotifyApi.getMultipleTrackPreviews(
          trackData
        );

        const updatedSongs = songs.map((song) => {
          const preview = previewUrls[song.id] ?? undefined;
          return {
            ...song,
            previewUrl: song.previewUrl || preview, // preview is now string | undefined
          };
        });
        ...
        setError(
          err instanceof Error ? err.message : "Failed to fetch preview URLs"
        );
